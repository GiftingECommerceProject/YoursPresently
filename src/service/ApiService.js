...
export default class ApiService {
    ...
    //     return role === 'VENDOR'
    // }
    static async loginCorporateUser(loginDetails) {
        const response = await axios.post(`${this.BASE_URL}/api/corporate-clients/login`, loginDetails);
        return response.data;
      }
    
      static async registerCorporateUser(corporateClientData) {
        const response = await axios.post(`${this.BASE_URL}/api/corporate-clients/register`, corporateClientData);
        return response.data;
      }
    
      static async getEmployees() {
        const response = await axios.get(`${this.BASE_URL}/api/corporate-clients/employees`, {
          headers: this.getHeader()
        });
        return response.data;
      }
    
      static isCorporateAuthenticated() {
        return localStorage.getItem('role') === 'CORPORATE'; // Use the role stored in localStorage
      }
    
      static Corporatelogout() {
        localStorage.removeItem('role');
      }
// Add this method to the ApiService

static async getEmployeesByCorporateClient(corporateClientId) {
    const response = await axios.get(`${this.BASE_URL}/api/corporate-clients/${corporateClientId}/employees`);
    return response.data;
}
static async addEmployee(corporateClientId, employeeData) {
    const response = await axios.post(`${this.BASE_URL}/api/corporate-clients/${corporateClientId}/employees`, employeeData);
    return response.data;
}
